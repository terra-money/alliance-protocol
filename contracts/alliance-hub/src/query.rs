use alliance_protocol::alliance_protocol::{AllPendingRewardsQuery, AssetQuery, PendingRewardsRes, QueryMsg, StakedBalanceRes};
#[cfg(not(feature = "library"))]
use cosmwasm_std::entry_point;
use cosmwasm_std::{to_binary, Binary, Deps, Env, Order, StdResult};
use cw_asset::{AssetInfo, AssetInfoBase, AssetInfoKey};

use crate::state::{ASSET_REWARD_DISTRIBUTION, ASSET_REWARD_RATE, BALANCES, CONFIG, USER_ASSET_REWARD_RATE, WHITELIST};
...
fn get_pending_rewards(deps: Deps, asset_query: AssetQuery) -> StdResult<Binary> {
    ...
    let asset_reward_rate = ASSET_REWARD_RATE.load(deps.storage, AssetInfoKey::from(asset_query.asset.clone()))?;
...
fn get_all_pending_rewards(deps: Deps, query: AllPendingRewardsQuery) -> StdResult<Binary> {
    ...
    let all_pending_rewards: StdResult<Vec<PendingRewardsRes>> = USER_ASSET_REWARD_RATE.prefix(addr.clone()).range(deps.storage, None, None, Order::Ascending).map(|item| {
        let (asset, user_reward_rate) = item?;
        let asset = asset.check(deps.api, None)?;
        let asset_reward_rate = ASSET_REWARD_RATE.load(deps.storage, AssetInfoKey::from(asset.clone()))?;
        let user_balance = BALANCES.load(deps.storage,(addr.clone(), AssetInfoKey::from(asset.clone())))?;
        let unclaimed_rewards = (asset_reward_rate - user_reward_rate) * user_balance;
